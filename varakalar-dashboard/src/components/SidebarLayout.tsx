...
const SidebarLayout: React.FC<SidebarLayoutProps> = ({
  ...
  return (
    <div className="h-dvh overflow-hidden bg-slate-50 dark:bg-neutral-950 text-slate-900 dark:text-neutral-100 flex flex-col font-sans antialiased">
      ...
      <div className="shrink-0 bg-slate-900 dark:bg-neutral-900 border-b border-slate-800 text-slate-200 px-4 py-2 text-xs sm:text-sm flex items-center justify-between z-50">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-md transition-transform duration-150 font-medium shadow-sm hover:scale-105 active:scale-95 text-xs"
            aria-label="Eski Klasik Görünüme Dön"
          >
      ...
      <div className="flex flex-1 min-h-0 relative overflow-hidden">
        ...
        <aside
          id="mobile-sidebar"
          className={`fixed lg:sticky top-0 left-0 z-40 h-dvh lg:h-full bg-slate-900 dark:bg-neutral-900 border-r border-slate-800 text-slate-300 flex flex-col transition-all duration-300 ${
        ...
        >
          ...
          <div className="flex-1 py-6 px-3 space-y-1.5 overflow-y-auto">
            {(!isCollapsed || mobileOpen) && (
              <div className="px-3 mb-2 text-[10px] font-semibold text-slate-400 uppercase">
                Ana Gezinti
              </div>
            )}
            ...
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn(
                      "flex items-center gap-3 px-3 py-3 rounded-xl transition-colors duration-150 font-medium text-sm",
                      isActive
            ...
            {(!isCollapsed || mobileOpen) && (
              <div className="pt-6 px-3 mb-2 text-[10px] font-semibold text-slate-400 uppercase">
                Yönetim & Eylemler
              </div>
            )}
            ...
            {user && profile?.role === 'admin' && (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-colors duration-150 font-medium text-sm text-purple-300 hover:bg-purple-950/40 hover:text-purple-200 border border-purple-800/30",
                  isCollapsed && !mobileOpen && "justify-center px-0"
                )}
                aria-label="Admin Paneli"
              >
                <ShieldCheck className="size-5 text-purple-400 shrink-0" aria-hidden="true" />
            ...
            {user && profile?.status === 'active' && (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-colors duration-150 font-medium text-sm text-emerald-300 hover:bg-emerald-950/40 hover:text-emerald-200 border border-emerald-800/30",
                  isCollapsed && !mobileOpen && "justify-center px-0"
                )}
                aria-label="Excel Dosyası Yükle"
              >
                <UploadCloud className="size-5 text-emerald-400 shrink-0" aria-hidden="true" />
          ...
          <div className="p-3 border-t border-slate-800/80 bg-slate-950/50">
            {user ? (
              <div className="flex items-center justify-between gap-2">
                {(!isCollapsed || mobileOpen) && (
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="size-8 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 flex items-center justify-center font-bold text-xs shrink-0">
                ...
                <button
                  onClick={() => signOut()}
                  className="p-2 text-slate-400 hover:text-rose-400 hover:bg-slate-800 rounded-lg transition-colors duration-150"
              ...
              <button
                onClick={onAuthClick}
                className="w-full py-2 px-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-semibold transition-colors duration-150"
              >
                Giriş Yap
              </button>
            )}
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0">
          ...
          <header className="h-16 shrink-0 bg-white dark:bg-neutral-900 border-b border-slate-200 dark:border-neutral-800 px-6 flex items-center justify-between z-50 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-base font-bold text-slate-900 dark:text-neutral-100 flex items-center gap-2 text-balance">
                  {location.pathname === '/' ? 'Zabıt Varakaları Genel Bakış' : 'Detaylı İhlal ve Ceza Tablosu'}
                </h1>
                <p className="text-xs text-slate-500 dark:text-neutral-400 hidden sm:block text-pretty">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button
                  ...
                  className="p-2 text-slate-600 dark:text-neutral-400 hover:bg-slate-100 dark:hover:bg-neutral-800 rounded-xl transition-colors duration-150 relative"
                ...
                {showNotifications && (
                  <div
                    ...
                    tabIndex={-1}
                    className="absolute right-0 mt-2 w-80 sm:w-96 bg-white dark:bg-neutral-900 rounded-2xl shadow-xl border border-slate-200 dark:border-neutral-800 p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-150"
                  >
                    ...
                    <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-100 dark:border-neutral-800">
                      <h4 className="font-bold text-sm text-slate-900 dark:text-neutral-100 flex items-center gap-2">
                        <Bell className="size-4 text-indigo-500" aria-hidden="true" />
                        <span>Sistem Bildirimleri</span>
                      </h4>
                      {unreadCount > 0 ? (
                        <span className="text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded-full border border-indigo-200 dark:border-indigo-800">
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto">
                      {notifications.map((n) => {
                        ...
                        <div
                          key={n.id}
                          className={cn(
                            "p-3 rounded-xl border transition-colors duration-150 text-xs",
                            isUnread
                        ...
                        >
                          <div className="flex items-center justify-between font-bold text-slate-900 dark:text-neutral-100 mb-1">
                            <span className="flex items-center gap-1.5">
                              {isUnread && <span className="size-1.5 rounded-full bg-indigo-500" aria-hidden="true" />}
                              {n.title}
                            </span>
                            <span className="text-[10px] text-slate-400 font-normal">{n.time}</span>
                          </div>
                          <p className="text-slate-600 dark:text-neutral-300 text-[11px] leading-relaxed text-pretty">
                            {n.desc}
                          </p>
                        </div>
                        );
              ...
              <button
                ...
                className="p-2 text-slate-600 dark:text-neutral-400 hover:bg-slate-100 dark:hover:bg-neutral-800 rounded-xl transition-colors duration-150"
                aria-label="Tema Değiştir"
              >
                {theme === 'dark' ? <Sun className="size-5 text-amber-400" aria-hidden="true" /> : <Moon className="size-5 text-slate-600" aria-hidden="true" />}
              </button>

              {/* View Switcher Toggle Button in Topbar */}
              <button
                onClick={onToggleViewMode}
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-neutral-800 hover:bg-slate-200 dark:hover:bg-neutral-700 text-slate-700 dark:text-neutral-300 rounded-xl text-xs font-semibold transition-colors duration-150 border border-slate-200 dark:border-neutral-700"
                aria-label="Görünüm Modunu Değiştir"
              >
                <SlidersHorizontal className="size-3.5 text-indigo-500" aria-hidden="true" />
